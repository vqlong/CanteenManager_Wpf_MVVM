...
{
    public class BillData : DataAccess, IBillData
    {
        private BillData() { }
        //Thằng này tự khởi tạo luôn mà không dùng Unity để TableData có thể sử dụng luôn
        private static readonly BillData instance = new BillData();
        public static BillData Instance { get { return instance; } }
        ...
        public List<Bill> GetListBillByDateAndPage(DateTime fromDate, DateTime toDate, int pageNumber = 1, int pageSize = 10)
        {
            string query = @$"WITH [temp] 
                              ...
                              SELECT * FROM (SELECT * FROM [temp] LIMIT {pageSize} * {pageNumber})
                              EXCEPT
                              SELECT * FROM (SELECT * FROM [temp] LIMIT {pageSize} * {pageNumber - 1});";
            ...
            if (list is DataView view)
            {
                for (int i = 0; i < view.Count; i++)
                {
                    bills.Add(new Bill
                    {
                        Id = Convert.ToInt32(view[i][0]),
                        DateCheckIn = Convert.ToDateTime(view[i][1]),
                        DateCheckOut = Convert.ToDateTime(view[i][2]),
                        TableId = (int)view[i][3],
                        Table = new Table { Name = view[i][4].ToString() },
                        Discount = (int)view[i][5],
                        TotalPrice = (double)view[i][6]
                    });
                }
            }
            ...
            if(list is DataView view)
            {
                for (int i = 0; i < view.Count; i++)
                {
                    bills.Add(new Bill
                    {
                        ...
                        DateCheckOut = Convert.ToDateTime(view[i][2]),
                        TableId = (int)view[i][3],
                        Table = new Table { Id = (int)view[i][3], Name = view[i][4].ToString() },
                        Discount = (int)view[i][5],
                        TotalPrice = (double)view[i][6]
                    });
                }
